...
func (o Operation) valid() bool {
	switch o {
	case Read, Derive, Disclose, Attribute, Aggregate, Match, Retain, Export, ShareOnRequest:
		return true
	}
	return false
}
...
func (g Grant) Validate() error {
	if err := ids(g.Actor, g.Recipient, g.Purpose); err != nil {
		return err
	}
...
func (r Rights) Allows(q PermissionRequest) bool {
	if r.Validate() != nil || q.Resource.Validate() != nil || q.Context.Validate() != nil || r.Revoked || q.Resource != r.Resource {
		return false
	}
	for _, g := range r.Grants {
		if g == q.Context {
...
func DeriveRights(resource ID, context Grant, sources []Rights) (Rights, error) {
	...
	if context.Operation != Derive || context.Validate() != nil || len(sources) == 0 {
		return Rights{}, fmt.Errorf("known derive context and sources required")
	}
	seen := map[ID]bool{}
	for _, source := range sources {
		if source.Resource == resource || seen[source.Resource] || !source.Allows(PermissionRequest{source.Resource, context}) {
			return Rights{}, fmt.Errorf("invalid or unauthorized source")
		}
		seen[source.Resource] = true
	}
	for _, g := range sources[0].Grants {
		allowed := true
		for _, source := range sources[1:] {
			if !source.Allows(PermissionRequest{source.Resource, g}) {
				allowed = false
				break
			}
		}
