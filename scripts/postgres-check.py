...
try:
    ...
    query = """SELECT (SELECT count(*) FROM dream.schema_versions WHERE version IN (1,2,3))=3 AND (SELECT max(version) FROM dream.schema_versions)=3
      AND (SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
        WHERE n.nspname='dream' AND c.relname IN ('observable_payloads','private_payloads','research_payloads','runtime_payloads','reader_scopes') AND c.relrowsecurity AND c.relforcerowsecurity)=5
